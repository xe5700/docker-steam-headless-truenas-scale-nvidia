...
function download_driver {
    ...
    if [[ ! -f "${USER_HOME:?}/Downloads/NVIDIA_${NVIDIA_DRIVER_VERSION:?}.run" ]]; then
        print_step_header "Downloading driver v${NVIDIA_DRIVER_VERSION:?}"
        aria2c -x16 -k1M -d/ -o tmp/NVIDIA.run "https://cn.download.nvidia.com/XFree86/Linux-x86_64/${NVIDIA_DRIVER_VERSION:?}/NVIDIA-Linux-x86_64-${NVIDIA_DRIVER_VERSION:?}.run"
        [[ $? -gt 0 ]] && print_error "Unable to download driver. Exit!" && return 1

        mv /tmp/NVIDIA.run "${USER_HOME:?}/Downloads/NVIDIA_${NVIDIA_DRIVER_VERSION:?}.run"
    fi
}
...
function install_nvidia_driver {
    ...
    # if [ "${nvidia_settings_version:-}X" != "${nvidia_host_driver_version:-}X" ]; then
        ...
        # else 
            print_step_header "Installing Legacy NVIDIA driver v${NVIDIA_DRIVER_VERSION:?} to match what is running on the host"
            chmod +x "${USER_HOME:?}/Downloads/NVIDIA_${NVIDIA_DRIVER_VERSION:?}.run"
            "${USER_HOME:?}/Downloads/NVIDIA_${NVIDIA_DRIVER_VERSION:?}.run" \
                --silent \
                --accept-license \
                --skip-depmod \
                --skip-module-unload \
...
if [[ ! -f "/usr/bin/nvidia-smi" ]]; then
    print_header "Install nvidia driver, so nvidia ?."
        ...
        (
            ...
            if [ ! -f "${USER_HOME:?}/Downloads/nvidia-patch.${NVIDIA_PATCH_VERSION:?}.sh" ]; then
                ...
                wget -q --show-progress --progress=bar:force:noscroll \
                    ...
                    "https://fastly.jsdelivr.net/gh/keylase/nvidia-patch/${NVIDIA_PATCH_VERSION:?}/patch.sh"
            fi
            if [ ! -f "${USER_HOME:?}/Downloads/nvidia-patch-fbc.${NVIDIA_PATCH_VERSION:?}.sh" ]; then
                ...
                wget -q --show-progress --progress=bar:force:noscroll \
                    ...
                    "https://fastly.jsdelivr.net/gh/keylase/nvidia-patch/${NVIDIA_PATCH_VERSION:?}/patch-fbc.sh"
            fi
            chmod +x \
